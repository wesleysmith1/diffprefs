...
class Player(BasePlayer):
    ...
    def set_payoff(self):
        ...
        if self.bomb:
            self.payoff = self.participant.vars['risk_payment'] = self.round_result = c(0)
        else:
            self.payoff = self.participant.vars['risk_payment'] = self.round_result = self.boxes_collected * Constants.box_value
